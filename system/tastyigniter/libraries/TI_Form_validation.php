    ...
    public function get_lat_lng($str, $post_item = 'address') {
        if (!empty($str) AND $post_data = $this->CI->input->post($post_item)) {
            if (is_array($post_data) AND !empty($post_data['address_1'])) {
                $url  = 'https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode(implode(", ", $post_data)) .'&sensor=false&libraries=geometry,places'; //encode $postcode string and construct the url query
                $ch = curl_init();
